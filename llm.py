#!/usr/bin/env python3
"""
llm: tiny OpenAI chat client with a streaming pager.

Usage:
  llm [options]

Options:
  -m MODEL --model=MODEL   Model name [env: LLM_MODEL, default: gpt-5.2].
  -u URL --base-url=URL    OpenAI-compatible base URL [env: LLM_BASE_URL, default: http://n:11434/v1].
  ...
  --log-file=PATH          Write debug logs to PATH; use --log-file= to disable [default: llm.log].
...
TAB = 8
HIST = os.path.expanduser("~/.llm_history")
...
TEXT_TOOL_HEADER = "You are running inside llm. llm may provide a text tool protocol for shell access."
TEXT_TOOL_FOOTER = """Runtime tool protocol for llm:
...
- After llm replies with <response>, answer normally in Markdown.
...
def tool_schema(name, description, props, required=()):
    return {
        "type": "function",
        "function": {
            "name": name,
            "description": description,
            "parameters": obj(props, required),
        },
    }
...
TOOLS = [
    tool_schema("shell", "Run a shell command line in the current working directory.", {
        "command_line": {"type": "string", "description": "The shell command line to execute."},
    }, ("command_line",)),
    tool_schema("write_file", "Create or replace a UTF-8 text file in the current working directory.", {
        "path": {"type": "string", "description": "Relative path to write."},
        "content": {"type": "string", "description": "Complete file contents."},
    }, ("path", "content")),
    tool_schema("read_files", "Read UTF-8 text files from the current working directory.", {
        "paths": {"type": "array", "items": FILE_SPEC},
    }, ("paths",)),
    tool_schema("search_replace", "Replace exact text in a UTF-8 text file.", {
    ...
    tool_schema("run_live", "Run a command attached to the terminal without capturing stdout/stderr.", {
        "command_line": {"type": "string", "description": "The command line to execute."},
    }, ("command_line",)),
    tool_schema("apply_patch", "Apply a unified diff patch using git apply after checking it.", {
        "patch": {"type": "string", "description": "Unified diff patch accepted by git apply."},
    }, ("patch",)),
    tool_schema("list_files", "List files with optional pattern and recursion.", {
...
def run_command(command_line, capture):
    name = "shell" if capture else "run_live"
    LOG.debug("%s start command=%r", name, command_line)
    try:
        p = subprocess.run(
            command_line,
            shell=True,
    ...
    LOG.debug("%s result %s", name, json.dumps(result))
    return result


def run_shell(command_line):
    return run_command(command_line, True)


def run_live(command_line):
    return run_command(command_line, False)
...
def run_tool(name, args):
    ...
    if name == "list_files":
        return list_files(args.get("path") or ".", args.get("pattern") or "*", bool(args.get("recursive", False)))
    if name == "apply_patch":
    ...
    return {"success": False, "tool": name, "stderr": f"Unknown tool: {name}"}
...
def execute_tool(call):
    ...
    args, error = tool_args(call)
    return error if error else run_tool(name, args)


def tool_display(name, args):
    if name in ("shell", "run_live"):
        return f"\n[{name}] $ {args.get('command_line', '')}\n"
...
def text_tool_args(request):
    tool = request.get("tool")
    command = request.get("command")
    if tool in ("shell", "run_live"):
        return {"command_line": command}
    if tool == "write_file":
        return {"path": request.get("path"), "content": command}
    if tool == "read_files":
        return {"paths": [p.strip() for p in command.splitlines() if p.strip()]}
    if tool == "list_files":
        return {
    ...
    if tool == "search_replace":
        return {
            ...
            "search": tag_text(command, "search"),
            "replace": tag_text(command, "replace"),
        }
    return {}
    ...
    try:
        while not cancel.is_set():
            ...
            if not tool_calls:
                ...
                if request:
                    ...
                    ui_events.put({"role": "display", "content": tool_display(tool, args)})
            ...
            for call in tool_calls:
                ...
                ui_events.put({"role": "display", "content": tool_display(name, args)})
                result = error or run_tool(name, args)
                ui_events.put({
                    "role": "display",
        ...
        ui_events.put({
            ...
            "content": (
                ...
                "Run `llm --list-models` or pass an installed model with `llm -m MODEL`.\n"
                f"Server said: {e}\n"
            ),
        })
        ui_events.put({"role": "done", "content": None})
...
def main():
    ...
    model = a["--model"] or os.getenv("LLM_MODEL") or os.getenv("LLMORE_MODEL") or DEFAULT_MODEL
    base_url = a["--base-url"] or os.getenv("LLM_BASE_URL") or os.getenv("LLMORE_BASE_URL") or DEFAULT_BASE_URL
